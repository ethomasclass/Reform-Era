...
import {Arrow, ColourReveal, Finish, Grid, Highlight, INK, JF, Loop, MaskData, Note, Picture, Place, Tag, Tint, Traced, Y} from './Kit';
...
export const JHCold: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{background: INK}}>
      ...
      <div style={{position: 'absolute', left: 1040, top: 770, fontFamily: JF.display, fontSize: 64, color: Y, textShadow: '0 3px 14px rgba(0,0,0,0.6)'}}>SAVANNAH, GEORGIA</div>
    ...
    <AbsoluteFill style={{background: '#1a1814', overflow: 'hidden'}}>
      <div style={{position: 'absolute', left: 960 - mid[0] * s, top: 560 - mid[1] * s, width: MAP.w * s, height: MAP.h * s,
        ...
        <svg style={{position: 'absolute', left: 0, top: 0, overflow: 'visible'}} width={MAP.w * s} height={MAP.h * s} viewBox={`0 0 ${MAP.w} ${MAP.h}`}>
          ...
          <path d={d} fill="none" stroke={Y} strokeWidth={12} strokeLinejoin="round" strokeLinecap="round" />
        </svg>
      </div>
      <AbsoluteFill style={{background: 'radial-gradient(ellipse at 50% 50%, transparent 40%, rgba(10,8,5,0.7) 100%)'}} />
      {[b, sv].map(([x, y], i) => (
        <div key={i} style={{position: 'absolute', left: x - 13, top: y - 13, width: 26, height: 26, borderRadius: '50%', background: Y, border: `4px solid ${INK}`, boxShadow: '0 0 0 3px rgba(255,225,26,0.4)'}} />
      ))}
      ...
      <div style={{position: 'absolute', left: 90, top: 215, width: 330, transform: 'rotate(-4deg)'}}>
        ...
        <div style={{position: 'absolute', left: 64, top: 16, width: 200, height: 40, background: INK, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: JF.display, fontSize: 30, color: Y}}>? ? ?</div>
      </div>
  ...
  return (
    <AbsoluteFill style={{background: '#121212'}}>
      <div style={{position: 'absolute', inset: 0, transform: 'rotate(-1.5deg)', transformOrigin: '1100px 590px'}}>
        ...
        <div style={{position: 'absolute', left: L(193), top: T(1612), width: (885 - 193) * ps, height: 42 * ps, background: Y, mixBlendMode: 'multiply'}} />
